...
function App() {
  ...
  const [unsplashBg, setUnsplashBg] = useState<{ imageUrl: string; creditLabel: string | null; creditLink: string | null } | null>(
    null,
  )
  ...
  const curatedWallpaperUrl = useMemo(() => {
    ...
    if (settings.wallpaperPlayback === "fixed")
      return list[settings.wallpaperFixedIndex % list.length]
  ...
  const greeting = getTablissStyleGreeting(time)
  ...
  return (
    <div className="relative min-h-dvh h-screen w-full overflow-hidden">
      ...
      {photoCredit?.creditLabel && photoCredit.creditLink && (
        <a
          href={photoCredit.creditLink}
          target="_blank"
          rel="noreferrer noopener"
          className="fixed bottom-4 left-4 z-20 text-[11px] text-white/55 hover:text-white/90 transition-colors duration-300 max-w-[min(90vw,280px)] truncate"
        >
          {photoCredit.creditLabel}
        </a>
      )}
      ...
      <div className="relative z-10 flex min-h-dvh w-full flex-col items-center px-4">
        ...
        >
          <button
            ...
            className={`tabliss-time-stack text-center select-none rounded-4xl px-6 sm:px-12 py-6 tabliss-zen-ease transition-[transform,background-color,box-shadow] duration-500 outline-none focus-visible:ring-2 focus-visible:ring-white/35 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent cursor-pointer border border-transparent ${
              isZenFocus ? "bg-white/6 shadow-[0_12px_48px_rgba(0,0,0,0.2)]" : "hover:bg-white/5 active:bg-white/7"
            }`}
          ...
          >
            <h1
              className={`tabliss-time-digital text-white drop-shadow-[0_2px_28px_rgba(0,0,0,0.5)] tabliss-zen-ease transition-[font-size,margin,line-height] duration-500 ${
                isZenFocus
                  ? "text-[clamp(3.5rem,15vw,9.5rem)] leading-[0.92] mt-0"
                  : "text-6xl sm:text-7xl md:text-8xl leading-none mt-0"
              }`}
            >
              {timeLine}
            </h1>
            <h2
              className={`tabliss-time-greeting text-white/90 tabliss-zen-ease transition-[font-size,margin,opacity] duration-500 ${
                isZenFocus
                  ? "text-[clamp(1.2rem,4vw,3rem)] mt-6 sm:mt-8 opacity-95"
                  : "text-xl sm:text-2xl md:text-3xl mt-4 opacity-92"
              }`}
            >
              {greeting}
            </h2>
